...
abstract class DeviceCommand implements Command {
    ...
    /** Returns every device name the server has told this agent about. */
    protected List<String> deviceNames() {
        return List.copyOf(printing.devices().names());
    }
     ...
     */
    protected Optional<Device> resolveDevice(String[] args) {
        if (args.length == 0) {
            out.println("Usage: " + usage());
            return Optional.empty();
        }
        Optional<Device> device = printing.devices().device(args[0]);
        if (device.isEmpty()) {
            List<String> known = deviceNames();
            out.println(known.isEmpty()
                    ? "Unknown device '" + args[0] + "'. This agent has no devices yet."
                    : "Unknown device '" + args[0] + "'. Known: " + String.join(", ", known));
        }
        return device;
    }
}
