 ...
 * <p>
 * A receipt printer is write-only for ThermAPI's purposes, so unlike a general serial
...
public class SerialHandler implements PrinterPort {
    ...
    /**
     * @param device the resolved settings for this printer
     * @param logger logger for connection lifecycle messages
     */
    public SerialHandler(DeviceSettings device, FoxLogger logger) {
    ...
    /** Returns the resolved settings this handler was created for. */
    ...
    public void startReconnectLoop() {
        if (shuttingDown || !device.serial().autoReconnect()) {
            return;
        }
        ...
        long delayMillis = device.serial().reconnectDelay().toMillis();
        logger.info("[" + deviceName() + "] Retrying every "
                + device.serial().reconnectDelay().toSeconds() + "s until the device returns");

        reconnectThread = Thread.ofVirtual()
                .name("thermapi-reconnect-" + deviceName())
    ...
    private boolean isExpectedDevice() {
        String portName = device.serial().port();
