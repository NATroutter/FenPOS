...
import { fetchRemoteImage } from "@/lib/assets/fetch-remote";
 ...
 * At the worst case it permits, 4096x4096, the decoded bitmap is 64 MB and the process peaked at
 ...
 * @returns the stored asset, carrying the URL for provenance
 ...
 *
 * @param bytes the image
 * @throws ApiError if the size cannot be read, or is beyond {@link MAX_IMAGE_DIMENSION}
 */
...
/** The shortest prefix carrying a PNG's size: signature, chunk length, chunk type, width, height. */
 ...
 * @returns its declared pixel size, or null if these are not bytes whose size can be read
 */
function declaredSize(bytes: Buffer): { width: number; height: number } | null {
	if (bytes.length >= PNG_HEADER_LENGTH && bytes.subarray(0, 8).equals(PNG_SIGNATURE)) {
		// IHDR is mandated to be the first chunk, so width and height sit at fixed offsets: eight
		// bytes of signature, four of chunk length, four of chunk type, then the two dimensions.
...
function jpegSize(bytes: Buffer): { width: number; height: number } | null {
	...
	while (at + 3 < bytes.length) {
		...
		if (JPEG_FRAME_MARKERS.includes(code)) {
			...
			return at + 7 <= bytes.length ? { height: bytes.readUInt16BE(at + 3), width: bytes.readUInt16BE(at + 5) } : null;
		}
		at += segment;
	}
...
async function requireNameFree(name: string): Promise<void> {
	...
	if (clash) {
		throw new ApiError("name_taken", `There is already an image called '${name}'.`, { field: "name" });
	}
}
