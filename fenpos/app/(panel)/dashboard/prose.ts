 ...
 *
 * Extracted for the same reason `sessionLifetimePhrase` (`lib/auth/session.ts`),
 * `signInThrottlePhrase` (`lib/auth/rate-limit.ts`) and `minimumLengthPhrase`
 * (`lib/auth/password-policy.ts`) were: this project has broken exactly this kind of sentence at
 * a boundary three times already — "0 MB", "1 distinct URLs", "1 hours" — and a hardcoded
 * `${WINDOW_HOURS}h` was itself already one boundary bug waiting to happen the day this setting
 * became configurable. Unlike those three, the count here never changes the unit's spelling — "h"
 * takes no plural — but the rule is to extract and pin down every sentence built from a configured
 * value, not only the ones known in advance to need it.
 *
