...
describe("panel permissions", () => {
	it("declares the complete set", () => {
		...
		// added with the "statistics" settings category, plus `stats:read`, which gates the Statistics
		// page itself. A count rather than a list, so adding one deliberately is a one-line change and
		// adding one accidentally is a failure.
		expect(PANEL_PERMISSION_IDS).toHaveLength(79);
	});
